...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(chat_id=update.effective_chat.id, text="Hello!", reply_markup=ReplyKeyboardMarkup(keyboard, resize_keyboard=True, one_time_keyboard=True))#Выводит текст
...
async def message_processing(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state[update.effective_user.id]['dia_stat'] == 1:
        state[update.effective_user.id]['first_num'] = int(update.effective_message.text)
    ...
    # elif 3 состояние, сохраняет знак считает результат и печатает его. Откатываешься к 0 состоянию
        ...
        state[update.effective_user.id]['second_num'] = int(update.effective_message.text)
        ...
        cursor.execute(f'SELECT id, last_calc FROM users WHERE id = {update.effective_user.id}')
        # print(cursor.fetchone()) получаем кортеж с 2 числами или None
        data = cursor.fetchone()
        if data:
            cursor.execute(f'UPDATE users SET last_calc = {result} WHERE id = {update.effective_user.id}')
        else:
            cursor.execute(f'INSERT INTO users VALUES({update.effective_user.id}, {result})')
        conn.commit()
        state[update.effective_user.id]['dia_stat'] = 0
...
if __name__ == '__main__':
    application = ApplicationBuilder().token('6837370132:AAGYe1_MY4r1TWLamGPM0VK1m4UaQ2VQFHw').build()
